...
const showDialog = (display: boolean) =>
  document
    .getElementById("pwa-dialog")
    ?.setAttribute("aria-hidden", String(!display));

addEventListener("load", () => {
  refreshSW = registerSW({
    immediate: true,
    onNeedRefresh: () => showDialog(true),
  });
});

export const onLoad: OnLoadFn = () => {
  document
    .getElementById("pwa-cancel")!
    .addEventListener("click", () => showDialog(false));
  document
    .getElementById("pwa-refresh")!
    .addEventListener("click", () => refreshSW?.(true));
};
</script>
